...
4. 对应的变球器在 `素材 → 元件 → 变球器 → 自定义变球器` 处，拖入场景后，在其 `检视器 → 元件链接` 中填写对应的 [`自定义玩家球名`](#ball-name)
...
该球变球器的 [矩形区域光](../item/light#light-type) 的发光颜色。
...
该球变球器的 [矩形区域光](../item/light#light-type) 的发光强度。
