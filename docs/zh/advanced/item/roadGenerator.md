...
- 点击跳转至 [`自定义顶点`](#custom-vertices) 路面生成器的使用方法
- [`自定义脚本`](../../script/roadGenerator) 路面生成器将在 `脚本` 板块中介绍
- 更高级的路面生成器：[`路点路面生成器`](#way-point-road-generator)
...
- `保存网格`：将当前路面生成器保存为网格，网格保存在 `素材 → 网格 → 默认` 中
...
当一个元件同时启用了 `路面生成器` 和 `路径` 组件，该元件会被判定为 `路点路面生成器`。
因此，如果想要制作移动路面，请创建一个新的、包含 `路径` 组件的元件来控制路面的移动。
