...
由于 _havok 物理引擎_ 本身的问题，当 **非凸体网格元件** 发生 [碰撞事件](#collision-enter) 或者 [触发器事件](trigger#trigger-event) 时，其网格的每一个面都会触发一次相应的事件。因此，当元件需要响应这些事件时，请尽量使用 `长方体` 和 `球体` 碰撞箱，或者启用 `凸面碰撞体` 选项。
